...
def normalize_outside(label: str) -> str:
    # în dataset-ul tău apare "0" ca outside; normalizăm la "O"
...
def load_id2label(path: Path):
    """
    Suporta:
      1) JSON dict {"0":"O", "1":"B-geo", ...}
      2) JSON list ["O","B-geo",...]
      3) linii simple: O\nB-geo\n...
      4) linii 2-coloane: "16 0" sau "0 16" sau "16\tO"
         -> detectăm automat care coloană e ID-ul (int)
    Returneaza dict: {id(int): label(str)}
    """
    if not path.exists():
        return None
    ...
    if two_col:
        ...
        for a, b in parsed_pairs:
            ...
            if a_is_int and not b_is_int:
                ...
                # ambele int? alegem primul ca id, al doilea ca label numeric (ex: 16 0)
                idx = int(a)
                lab = b
            else:
                # ambele non-int (ciudat) -> fallback: index
                continue

            id2label[idx] = normalize_outside(lab)
        return id2label if id2label else None

    # 3) fallback: câte un label pe linie
...
def merge_wordpieces(tokens, labels, scores):
    ...
    for tok, lab, sc in zip(tokens, labels, scores):
        if tok.startswith("##") and merged_tokens:
            ...
            # păstrăm label/scor de la primul subtoken
        else:
...
def main():
    ...
    print("\nTip: schimbă variabila TEXT din fișier și rulează iar 🙂")
