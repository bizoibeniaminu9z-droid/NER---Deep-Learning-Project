...
# 2) Facem un vocab de label-uri (string -> id)
unique_labels = sorted({tag for sent_tags in labels for tag in sent_tags})
label2id = {lab: i for i, lab in enumerate(unique_labels)}
id2label = {i: lab for lab, i in label2id.items()}

# 3) Încărcăm tokenizerul BERT
tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)

# Alegem o propoziție de test
i = 0
words = sentences[i]
word_labels = labels[i]

print(f"Propoziție [{i}] (primele 30 cuvinte):")
print(words[:30])
print(word_labels[:30])
print()

# 4) Tokenizăm "pe cuvinte" (IMPORTANT: is_split_into_words=True)
enc = tokenizer(
    words,
    is_split_into_words=True,
    truncation=True,
    return_offsets_mapping=True,  # doar ca să vezi ce se întâmplă; îl scoatem la training
)

word_ids = enc.word_ids()  # pentru fiecare token, la ce index de cuvânt aparține (sau None)
...
for wid in word_ids:
    if wid is None:
        ...
        aligned_label_ids.append(label2id[word_labels[wid]])  # primul sub-token primește label-ul cuvântului
    else:
        aligned_label_ids.append(-100)  # restul sub-tokenilor ignorați
    prev_word_id = wid

# 6) Print frumos: token -> word -> gold label / -100
...
# Mică verificare
print("\nVerificare rapidă:")
print("Număr tokens:", len(tokens))
print("Număr aligned labels:", len(aligned_label_ids))
print("Contează la loss (labels != -100):", sum(l != -100 for l in aligned_label_ids))
