...
# păstrăm simplu, dar corect:
BATCH_SIZE = 32
EPOCHS = 8
LR = 1e-3
WEIGHT_DECAY = 1e-4
MAX_LEN = 128  # tăiem propozițiile prea lungi (rapid + ok)

EMB_DIM = 100     # embeddings random
HID_DIM = 128
DROPOUT = 0.2
...
def read_conll_csv(csv_path: Path):
    ...
    Sentence # are NaN pe rândurile care continuă aceeași propoziție.
    """
...
def collate_fn(batch, pad_word_id=0, pad_label_id=0):
    """
    batch: list[(x_seq, y_seq)] cu lungimi diferite
    -> pad la max_len din batch
    """
    xs, ys = zip(*batch)
...
class LinearCRF(nn.Module):
    """
    Linear-chain CRF (batched), with:
      - transitions[from_tag, to_tag]
      - start_transitions[to_tag]
      - end_transitions[from_tag]
    Works on:
      emissions: [B, T, C]
      tags:      [B, T]
      mask:      [B, T] bool
    ...
    def _compute_score(self, emissions, tags, mask):
        ...
        # end transition for last real tag
        lengths = mask.long().sum(dim=1)  # number of valid tokens
    ...
    def _compute_log_partition(self, emissions, mask):
        ...
        for t in range(1, T):
            ...
            # alpha_prev (from) + trans(from->to) + emit(to)
            scores = alpha.unsqueeze(2) + trans + emit_t  # [B, C, C]
            new_alpha = log_sum_exp(scores, dim=1)        # sum over from -> [B, C]

            alpha = torch.where(mask[:, t].unsqueeze(1), new_alpha, alpha)

        alpha = alpha + self.end_transitions
        return log_sum_exp(alpha, dim=1)  # [B]
    ...
    def _viterbi_decode(self, emissions, mask):
        ...
        for t in range(1, T):
            ...
            best_score, best_path = next_score.max(dim=1)   # over "from" -> [B, C]
            best_score = best_score + emissions[:, t, :]    # add emission(to)
...
class BiLSTMCRF(nn.Module):
    ...
    def loss(self, x, y, mask, lengths):
        ...
        # mask/y trebuie tăiate la dim T' (uneori pad_packed_sequence dă T' = max(lengths))
...
def evaluate(model, loader, id2label, device):
    ...
    with torch.no_grad():
        for x, y, mask, lengths in loader:
            ...
            pred_ids = model.decode(x, mask, lengths)  # list[list[int]] with real lengths
...
def main():
    ...
    pad_label_id = label2id["O"]  # folosim O ca pad label, dar mask îl elimină din loss/eval
